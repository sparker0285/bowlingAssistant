...
2. **Score sheet split symbol S# and Data for Set table condensing:**
...
**Update (Follow-up):** Save Edits still had no effect; e.g. changing 3,6 leave to 3,10 (split) reverted on next shot or refresh.

4. **Save Edits: use st.form so edited data is returned on submit:**
    *   **Reasoning:** Relying on session state or a button callback after rerun was not reliable—the data_editor value was not available or was stale when the save logic ran. Using a form ensures the data_editor and the Save button are submitted together, so in the same run we receive the edited dataframe as the data_editor’s return value when the form is submitted.
